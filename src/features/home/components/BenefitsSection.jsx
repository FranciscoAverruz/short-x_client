...
const benefitsData = [
  ...
  },
  {
    icon: FaShieldAlt,
    title: 'Seguro',
    description: 'Protege tus enlaces con seguridad avanzada y sin preocupaciones.',
  },
  {
    icon: FaChartBar,
    title: 'Con estadísticas avanzadas',
    description: 'Obtén detalles sobre el rendimiento de tus enlaces, clics, ubicación geográfica y más.',
  },
  {
    icon: FaPalette,
    ...
    <section className="w-full flex flex-col items-center">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 w-3/4 lg:w-full max-w-6xl">
        {benefitsData.map((benefit, index) => {
          ...
          return (
            <div key={index} className="flex flex-col justify-center items-center text-center transition-transform duration-300 group hover:scale-105 cursor-pointer">
              ...
              <section className="h-28 md:h-36">
                <h3 className="text-light-Title dark:text-dark-Title font-bold mt-5">{benefit.title}</h3>
                <p className="text-light-grlText dark:text-dark-grlText mt-0 md:mt-2">{benefit.description}</p>
              </section>
            </div>
          );
        })}
