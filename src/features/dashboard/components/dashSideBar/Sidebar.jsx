...
export function Sidebar({ expanded, setExpanded }) {
  ...
  return (
    <section className="absolute lg:relative z-[40]">
      ...
      <nav
        className={`relative flex sectionBg flex-col justify-between h-[calc(100vh-4rem)] pt-3 border-r dark:border-none transition-all duration-100 ${
          expanded
            ? "lg:bg-auto w-full left-0"
            : "opacity-0 lg:opacity-100 -left-40 lg:left-0 w-0 lg:w-auto"
        }`}
      >
        <section className="">
          <section className="border-b dark:border-dark-btnMenuHoverBg flex px-3 py-1">
            <article
              className={`flex justify-between items-center overflow-hidden transition-all duration-300 ${
                expanded ? "w-52 ml-3" : "w-0 left-52"
              }`}
            >
              <div className="flex flex-col h-fit pb-2">
                <div className="font-semibold text-light-Title dark:text-dark-Title text-xl/4 mb-0 py-0">
                  {username}
                </div>
                <div className="text-xs paragraphText my-0">{email}</div>
              </div>
            </article>
          </section>
