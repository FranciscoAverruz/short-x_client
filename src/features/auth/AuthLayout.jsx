  ...
  return (
    <section className={`flex justify-center w-full lg:w-[50%] ${className}`}>
      <main className="grlContainer innerlight flex flex-col md:flex-row p-5 gap-5 relative w-full my-5 md:my-5">
        <aside
          className={`absolute -left-[30px] -bottom-[4.4rem] w-20 z-0 md:z-[2] scale-y-[-1] -rotate-90 ${borderClasses?.left}`}
        >
          <img src={borderSrc} alt="border left" />
        </aside>
        <aside
          className={`absolute -right-[30px] -top-[4.4rem] w-20 z-0 md:z-[2] scale-y-[-1] rotate-90 ${borderClasses?.right}`}
        >
        ...
        <aside className="relative shadow-lg rounded-xl innerlight p-3 md:p-10 h-20 md:h-auto w-full md:w-[30%]">
          ...
          <h2 className="absolute flex title top-0 left-0 h-full w-full items-center pl-5 md:hidden z-10 bg-gradient-to-tr from-light-bg/80 dark:from-dark-bg text-light-Title dark:text-dark-Title">
            {title}
          </h2>
        </aside>
